...
export default function PendingUsersPage() {
    ...
    }, [isAuthenticated, page, limit, role]);
    ...
    const loadPendingUsers = async () => {
        try {
            ...
            console.log('🔍 Fetching pending users with params:', params);
            const response = await apiClient.getUsers(params);
            console.log('📦 Pending users response:', response);
            console.log('📊 Response data:', response.data);

            // Log each user's data in detail
            if (response.data && response.data.length > 0) {
                ...
                response.data.forEach((user, index) => {
                    ...
                    console.log('Phone Number (direct):', user.phoneNumber);
                    console.log('Doctor Profile:', user.doctorProfile);
                    console.log('Doctor Phone:', user.doctorProfile?.phoneNumber);
                    console.log('Patient Profile:', user.patientProfile);
                    console.log('Patient Emergency Contact:', user.patientProfile?.emergencyContactNumber);
                });
            } else {
        ...
        <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
            ...
            <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                ...
                <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6 mb-6">
                    <h2 className="text-lg font-semibold text-gray-900 dark:text-white mb-4">Filter Pending Users</h2>
                    ...
                    <div className="flex gap-2 mt-4 pt-4 border-t border-gray-200 dark:border-gray-700">
                        <button
                            onClick={() => exportUsersToPDF(users, 'Pending_Users_Report')}
                        ...
                        <button
                            onClick={() => exportUsersToExcel(users, 'Pending_Users_Export')}
                            disabled={users.length === 0}
                ...
                <div className="bg-white dark:bg-gray-800 rounded-lg shadow overflow-hidden">
                    {loadingUsers ? (
                        <div className="text-center py-12">
                            <div className="text-gray-600 dark:text-gray-400">Loading pending users...</div>
                        </div>
                    ) : users.length > 0 ? (
                        <>
                                ...
                                <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                                    {users.map((user) => (
                                        <tr key={user._id}>
                                            ...
                                            <td className="px-6 py-4 whitespace-nowrap">
                                                <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${(user.status === 'approved' || user.status === 'active')
                        ...
                        <div className="text-center py-12">
                            <div className="text-gray-600 dark:text-gray-400">No pending users found</div>
                        </div>
                    )}
                </div>
            </main>
